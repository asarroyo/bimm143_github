...
Now lets try ggplot. First I need to install the package using
`install.packages("ggplot2")`

> **N.B** We never run an `install.packages()` in a code chunk otherwise
> we will re-install needlessly everytime we render our document.
...
Read some data on the effectes of GLP-1 inhibitor (drug) in gene
expression values:
...
Lets color by `state` up, down or not change.
...
ggplot(gapminder) +
  ...
  geom_point() +
  scale_color_manual(values= c("purple", "blue", "pink", "red", "green"))
```
