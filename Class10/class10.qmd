...
X-Ray: 81% and Electron Microscopy: 13%

> Q2: What proportion of structures in the PDB are protein?

```{r}
protein_total <- sum(stats[c("Protein (only)", "Protein/NA"), "Total"])
protein_prop <- protein_total / sum(stats[, "Total"])

round(protein_prop, 2)
```
...
> Q. Generate and insert an image of the HIV-Pr cartoon colored by secondary structure, showing inhibitor (ligand) in ball and stick.
...
We can use the **remotes** package to install any R packages from GitHub.
...
Now we can download these "top hits" these will all be ADK structures in the ADK database.
...
Now we can use the `install()` function from this package luke this:
