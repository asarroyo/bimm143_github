...
Let's make up some input data for clustering where we know where these natural "clusters" are
...
> Q. What component of the results object details the cluster memebership vector (i.e. our main result of which points lie in which cluster)? 

```{r}
km$cluster
```

> Q. Plot our clustering results which points colored by cluster and also add the cluster centers as new points colored blue?
...
> Q. Run `kmean()` again and this time produce 4 clusters (and call your result object `k4`) and make a results figure like above?
...
> Q. Lets try different number of K (centers) from 1 to 30 and see what the best result is?
...
Unlike `kmeans()` (which does all the work for you) you cant just pass `hclust()` our raw input data. It needs a "distance matrix" like the one returned from the `dist()` function.
...
To make one of main PCA result figures we turn to `pca$x` the scores along our new PCs. This is called "PC plot" or "score plot" ot "Ordination plot"...
